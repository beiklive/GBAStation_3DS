...
If a sibling `switchVK/nvk-switch-26.1.4` or
`switch-nvk/nvk-switch-26.1.4` package exists, simply run:

```sh
bash build_local.sh
```

The canonical nxvk SDK is built by switchVK's Linux GitHub Actions/container
flow. On Windows/MSYS2, extract the release beside the repositories before
building GBAStation. The normal directory layout is:
...
```sh
SWITCH_NVK_ROOT=/path/to/nvk-switch-26.1.4 bash build_local.sh
```
