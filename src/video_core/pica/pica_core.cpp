...
void PicaCore::ProcessCmdList(PAddr list, u32 size, bool ignore_list) [[hot]] {
    ...
    if (head == nullptr) [[unlikely]] {
        LOG_ERROR(HW_GPU, "Commandlist points to invalid physical address 0x{:08X}", list);
    ...
    while (cmd_list.current_index < cmd_list.length) {
        ...
        if (cmd_list.current_index + 1 >= cmd_list.length) [[unlikely]] {
            LOG_ERROR(HW_GPU, "Commandlist 0x{:08X} ends with an incomplete command header",
                      cmd_list.addr);
            break;
        }
        ...
        if (count == 0)
            continue;
        if (count > cmd_list.length - cmd_list.current_index) [[unlikely]] {
            LOG_ERROR(HW_GPU,
                      "Commandlist 0x{:08X} register 0x{:03X} requests {} extra words, only {} "
                      "remain",
                      cmd_list.addr, static_cast<u32>(header.cmd_id), count,
                      cmd_list.length - cmd_list.current_index);
            break;
        }
...
void PicaCore::HandleSpecialReg(u32 id, u32 value, bool& stop_requested) {
    ...
    case PICA_REG_INDEX(pipeline.command_buffer.trigger[1]): {
        ...
        if (head == nullptr) [[unlikely]] {
            LOG_ERROR(HW_GPU, "Nested commandlist points to invalid physical address 0x{:08X}",
                      addr);
            stop_requested = true;
            break;
        }
...
void PicaCore::WriteInternalRegBatch(u32 id, const u32* values, u32 count, u32 mask,
    ...
    if (reg_impl_flags_lut[id].NeedsSpecialHandling() &&
        !reg_impl_flags_lut[id].SupportsBatch()) [[unlikely]] {
        for (u32 i = 0; i < count && !stop_requested; ++i) {
            WriteInternalReg(id, values[i], mask, stop_requested);
        }
        return;
    }
