...
void MasterSemaphoreTimeline::Wait(u64 tick) {
    ...
    const vk::Result result = instance.GetDevice().waitSemaphoresKHR(&wait_info, WAIT_TIMEOUT);
    if (result != vk::Result::eSuccess) {
        UNREACHABLE_MSG("Timeline semaphore wait failed with error {}", vk::to_string(result));
    }
    Refresh();
}
...
MasterSemaphoreFence::~MasterSemaphoreFence() {
    ...
    while (!wait_queue.empty()) {
        device.destroyFence(wait_queue.front().first);
        wait_queue.pop();
    }
    std::ranges::for_each(free_queue,
                          [device](auto fence) { device.destroyFence(fence); });
}
