...
struct LsfgNxRuntime {
    ...
    VkResult present(VkQueue queue, const VkPresentInfoKHR& info) {
        ...
        if (realFrameIndex == 0) {
            ...
            slot.pending = true;
            ++realFrameIndex;
            return presentOriginal(queue, info, originalImageIndex, slot.original.handle());
        }
        ...
        const VkResult originalResult =
            presentOriginal(queue, info, restoredImageIndex, slot.original.handle());
...
private:
    ...
    VkResult presentGenerated(VkQueue queue, const VkPresentInfoKHR& original,
        ...
        // The application's present-id/timing chain describes its one real
        // frame.  It must not be submitted a second time for the generated
        // frame, otherwise drivers may reject duplicate present IDs.
        generated.pNext = nullptr;
...
extern "C" LsfgNxRuntime *lsfg_nx_create(const LsfgNxCreateInfo *info) {
    ...
    try {
        ...
        LOG_ERROR(Render_Vulkan, "LSFG initialization failed: {}", error.what());
    } catch (...) {
        LOG_ERROR(Render_Vulkan, "LSFG initialization failed with an unknown error");
    }
    return nullptr;
}
...
extern "C" bool lsfg_nx_present(LsfgNxRuntime *runtime, VkQueue queue,
    ...
    try {
        *result = runtime->present(queue, *presentInfo);
        ...
        LOG_ERROR(Render_Vulkan, "LSFG presentation failed: {}", error.what());
    } catch (...) {
        LOG_ERROR(Render_Vulkan, "LSFG presentation failed with an unknown error");
    }
