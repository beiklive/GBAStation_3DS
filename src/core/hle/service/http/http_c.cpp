...
void Context::MakeRequestSSL(httplib::Request& request, const Common::URLInfo& url_info,
    ...
    // The Switch build currently uses cpp-httplib without an OpenSSL backend. Keep the
    // emulated HTTP service operational for plain HTTP and fail HTTPS requests cleanly
    // instead of pulling LibreSSL/OpenSSL into the NRO.
    (void)request;
    (void)url_info;
    (void)pending_headers;
    LOG_ERROR(Service_HTTP, "HTTPS is unavailable on this Switch build");
    state = RequestState::Completed;
#else
    httplib::Error error{-1};
    X509* cert = nullptr;
