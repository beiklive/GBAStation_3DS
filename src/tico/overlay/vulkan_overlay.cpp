...
constexpr std::array<const char*, 2> FontPaths{{
    ...
    "sdmc:/GBAStation/fonts/font.ttf",
}};
...
bool Init(Vulkan::RendererVulkan& renderer) {
    ...
    for (const char* path : FontPaths) {
        if (ImFont* font = io.Fonts->AddFontFromFileTTF(
                path, 30.0f, &font_config, io.Fonts->GetGlyphRangesChineseSimplifiedCommon())) {
            io.FontDefault = font;
            LOG_INFO(Render_Vulkan, "{} loaded menu font {}", Tag, path);
            break;
        }
    }
    if (!io.FontDefault) {
        io.FontDefault = io.Fonts->AddFontDefault();
        LOG_WARNING(Render_Vulkan, "{} Chinese font unavailable; using ImGui default", Tag);
    }
...
void Update(PadState* pad) {
    ...
    const bool combo = (held & HidNpadButton_ZL) && (held & HidNpadButton_ZR);
