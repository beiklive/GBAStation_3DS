...
struct TicoSwitchJit {
    Jit jit{};
};

void tico_switch_random_bytes(void* output, size_t size) {
    randomGet(output, size);
}

void* tico_switch_jit_create(size_t size, void** rw_addr, void** rx_addr) {
    auto* handle = new (std::nothrow) TicoSwitchJit{};
...
int tico_switch_jit_set_writable(void* opaque) {
    auto* handle = static_cast<TicoSwitchJit*>(opaque);
    return !handle || jitTransitionToWritable(&handle->jit) != 0;
}

int tico_switch_jit_set_executable(void* opaque) {
    auto* handle = static_cast<TicoSwitchJit*>(opaque);
...
int tico_switch_jit_sync_range(void* opaque, size_t offset, size_t size) {
    auto* handle = static_cast<TicoSwitchJit*>(opaque);
...
void tico_switch_jit_close(void* opaque) {
    auto* handle = static_cast<TicoSwitchJit*>(opaque);
    if (!handle) {
        return;
    }
...
int getpwuid_r(uid_t uid, struct passwd* pwd, char* buffer, size_t buffer_size,
    ...
    constexpr char home[] = "sdmc:/tico/system/3ds";
